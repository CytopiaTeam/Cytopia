...
protected:
  ...
  using MyInitialSpeed = std::conditional_t<
    ...
    std::conditional_t<
      ...
      MaxSpeed,
      typename CarSpecification::Speed
      >
    >;
...
struct DefaultAccidentHandler { Behaviour handle() { return DriveSlow; } };
struct DefaultTrafficHandler { Behaviour handle() { return DriveSlow; } };
struct DefaultSlowHandler { Behaviour handle() { return DriveSlow; } };
...
struct NormalCarSpecification
{
  ...
  using EventHandlers = TypeList<
                          DefaultAccidentHandler,
                          onTraffic, 
                          DefaultSlowHandler>;
  using Speed = Constant<50, CarSpeed>;
};

class NormalCar : public AbstractCar<NormalCarSpecification> { /* We can add more things here */ };
...
class ChaoticCar : public AbstractCar<ChaoticCarSpecification> { /* We can add more things here */ };
