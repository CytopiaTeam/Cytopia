...
struct IsStrongEquatableType : public std::false_type { };
...
public:
  ...
  IEquatable()
  {
    ...
    static_assert(std::is_same_v<FunctorType, std::size_t() const noexcept>, "You must implement std::size_t Hash() const noexcept and make it accessible to IEquatable");
  }


  inline constexpr bool operator!=(const IEquatable<Type>& other) const noexcept
  {
