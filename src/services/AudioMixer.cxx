...
AudioMixer::AudioMixer(GameService::ServiceTuple &context)
    : GameService(context), m_LoadSoundThread(&AudioMixer::loadAllSounds, this)
{
...
void AudioMixer::loadAllSounds()
{
  try
  {
    ...
    /* Load all Music */
    loadSoundtrack(audioConfig.Music.begin(), audioConfig.Music.end(), [](const string &name, Mix_Chunk *chunk) {
      return new Soundtrack{SoundtrackID{name}, ChannelID{0}, chunk, RepeatCount{0}, true, false, true, true};
    });

    /* Load all Sounds */
    loadSoundtrack(audioConfig.Sound.begin(), audioConfig.Sound.end(), [](const string &name, Mix_Chunk *chunk) {
      return new Soundtrack{SoundtrackID{name}, ChannelID{-1}, chunk, RepeatCount{0}, false, false, true, true};
    });

    /* Generate m_GetSound mapping */
    std::for_each(m_Soundtracks.begin(), m_Soundtracks.end(), [this](SoundtrackUPtr &ptr) { m_GetSound[ptr->ID] = &ptr; });
...
void AudioMixer::loadSoundtrack(Iterator begin, Iterator end, CallbackType createSoundtrack)
{
  std::for_each(begin, end, [this, &createSoundtrack](auto &kvp) {
    if (running)
    {
      ...
      alBufferData(m_Soundtracks.back()->buffer, format, m_Soundtracks.back()->Chunks->abuf, m_Soundtracks.back()->Chunks->alen,
                   44100);
