...
void Text::createTextTexture(const std::string &text, const SDL_Color &textColor)
{
  ...
  fs::path fontFName = SDL_GetBasePath() + Settings::instance().fontFileName.get(); 
  
  if(!fs::is_regular_file(fontFName))
    throw ConfigurationError(TRACE_INFO "File " + fontFName.string() + " doesn't exist");
  
  TTF_Font *font = TTF_OpenFont(fontFName.string().c_str(), m_fontSize);

  if (!font)
    throw FontError(TRACE_INFO "Failed to load font " + fontFName.string() + ": " + TTF_GetError());
