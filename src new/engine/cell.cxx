...
bool Cell::isInCell(Point clickedCoords)
{
  ...
  int cellCenterX = screenCoordinates.getX() + (tilesize * 0.5 * _zoomLevel);
  int cellCenterY = screenCoordinates.getY() + (tilesize * 0.75  * _zoomLevel);

  int distanceX = abs((clickedCoords.getX()) - cellCenterX);
  int distanceY = abs((clickedCoords.getY()) - cellCenterX);

  float calc = (distanceX / (tilesize * 0.5 * _zoomLevel)) + (distanceY / (tilesize * 0.25 * _zoomLevel));
  if (calc <= 1.0) {

    std::cout << "--------------------" << std::endl;
    std::cout << "Calculated : " << calc << " My zoomlevel = " << _zoomLevel << std::endl;
    std::cout << "dx, dy : " << distanceX << " , " << distanceY << std::endl;
    std::cout << "Calculated dx value: " << (distanceY / (tilesize)) << std::endl;
    ...
    std::cout << "My SCREEN Coordinates are: " << screenCoordinates.getX() << " , " << screenCoordinates.getY() <<
      "\t and my Center = " << cellCenterX << " , " << cellCenterY << std::endl;
